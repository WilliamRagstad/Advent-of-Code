...
// --- Part Two ---
(** Conclusion:
- Update the valueToInt function to treat J as the weakest card.
- Modify classifyHand to consider J cards as wildcards that can form the best possible hand type.
- In the compareHands function, ensure that J cards are treated as the weakest for tie-breaking.
**)
...
example_assert 2 1 5905 (example_1 |> lines |> Array.map parseHand |> solve_p2)
let p2 = input |> lines |> Array.map parseHand |> solve_p2
printfn "P2: %A" p2
// Tries:
// 87263515 - correct
